...
@section('title', 'Proposal Skripsi - Repositori Dosen')

@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800 flex items-center">
        <i class="fas fa-file-word text-yellow-600 mr-2"></i>Proposal Skripsi Mahasiswa
    </h1>
    <p class="text-gray-600 mt-1">Daftar proposal skripsi mahasiswa</p>
</div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-semibold text-gray-900">{{ $stats['total_proposal'] }}</p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-lg shadow p-6">
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="p-4 border-b">
        <h3 class="text-lg font-semibold text-gray-800">
            <i class="fas fa-list text-yellow-600 mr-2"></i> Daftar Proposal Skripsi
        </h3>
    </div>
    <div class="p-4 overflow-x-auto">
        <table class="w-full min-w-max" id="proposalTable">
            ...
            <tbody id="proposalTableBody">
                ...
                @foreach($dosens as $dosen)
                    @foreach($dosen->proposalSebagaiPembimbing1 as $skripsi)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="py-3 px-4">{{ $no++ }}</td>
                        <td class="py-3 px-4">
                            <a href="{{ route('admin.file.proposal.dosen', $dosen) }}" class="text-blue-600 hover:underline">
                                {{ $dosen->nama }}
                            </a>
                        </td>
                    ...
                    @endforeach
                    @foreach($dosen->proposalSebagaiPembimbing2 as $skripsi)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="py-3 px-4">{{ $no++ }}</td>
                        <td class="py-3 px-4">
                            <a href="{{ route('admin.file.proposal.dosen', $dosen) }}" class="text-blue-600 hover:underline">
        ...
        @if($no == 1)
            <div class="text-center py-8">
                ...
                <p class="text-gray-500">Belum ada proposal yang tersedia.</p>
            </div>
        @endif
    </div>
</div>
